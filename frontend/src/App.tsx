...
function App() {
  ...
  const chatMutation = useMutation({
    mutationFn: (query: string) => apiService.chat(userId, query).then(response => response.data),
    onSuccess: (data) => {
      ...
      if (autoPlayAudio) handleSpeak(data.response);
      toast.success(`Response generated (${data.tokens_used} tokens)`);
      
      // Invalidate and refetch workflow status if needed
      if (data.workflow_id) {
